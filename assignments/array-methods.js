...
// Problem 1
// Get each runner's email address, and sort them alphabetically
let emailList = [];
...
// Get a list of runners who donated $100 or more

let largeDonors = runners.filter(currentValue => currentValue.donation >= 100);

console.log(largeDonors);

// Problem 3

// Return a reversed version of every runner's full name.

let reversedRunners = runners.map(currentValue => `${currentValue.last_name.split('').reverse().join('')} ${currentValue.first_name.split('').reverse().join('')}`);

console.log(reversedRunners);